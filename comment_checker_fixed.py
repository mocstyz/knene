...
class CommentViolationChecker:
    ...
    def check_block_comment_violations(self, lines: List[str], file_path: str) -> List[Dict]:
        ...
        for i, line in enumerate(lines):
            ...
            # 检测块注释中间的行（排除已经是//注释的情况）
            if in_block_comment and not stripped.startswith('//'):
                violations.append({
                    'file': file_path,
                    'line': i + 1,
                    'type': 'BLOCK_COMMENT_VIOLATION',
            ...
            if stripped.startswith('//'):
                ...
                for j in range(i + 1, min(i + 5, len(lines))):  # 向下查找最多5行
                    ...
                    if candidate_line and not candidate_line.startswith('//') and not candidate_line.startswith('@'):
                        ...
                        is_code_line = (
                            # 包含Java关键字
                            any(keyword in candidate_line.split()[0] for keyword in java_keywords if candidate_line.split()) or
                            # 包含Java类型
                            any(type_name in candidate_line for type_name in java_types) or
                            # 包含变量赋值
                            ('=' in candidate_line and not candidate_line.startswith('//')) or
                            # 包含方法调用
                            (candidate_line.count('(') > 0 and candidate_line.count(')') > 0) or
                            # 包含花括号
                            ('{' in candidate_line or '}' in candidate_line)
                        )
                ...
                if next_code_line:
                    ...
                    # 4. 特殊情况：注释缩进超过代码缩进太多
                        ...
                        violations.append({
                            ...
                            'line': i + 1,
                            'type': 'MISALIGNED_COMMENT_VIOLATION',
                            'message': f'注释缩进与下方代码差异过大（相差{current_indent - next_line_indent}个空格）',
                            'content': line.strip()
                        })
