...
('verification.health.score.threshold.critical', '60.0', 'DECIMAL', '健康评分严重阈值（低于此值触发严重告警）', 'ALL'),
('verification.health.score.threshold.warning', '80.0', 'DECIMAL', '健康评分警告阈值（低于此值触发警告）', 'ALL'),
('verification.execution.timeout.seconds', '300', 'INTEGER', '验证脚本执行超时时间（秒）', 'ALL'),
('verification.max.retry.count', '3', 'INTEGER', '验证失败最大重试次数', 'ALL'),
...
('verification.retention.days', '90', 'INTEGER', '验证结果保留天数', 'ALL'),
...
-- 验证执行概览视图
CREATE VIEW v_verification_execution_overview AS
SELECT
    e.execution_id,
    e.execution_type,
    e.trigger_source,
...
-- 最新验证结果视图
CREATE VIEW v_latest_verification_results AS
SELECT
    vr.*,
...
-- 模块验证统计视图
CREATE VIEW v_module_verification_stats AS
SELECT
    module_name,
    ...
    AVG(execution_duration_seconds) as average_duration,
    MAX(start_time) as last_execution_time,
    MIN(start_time) as first_execution_time
FROM verification_executions
