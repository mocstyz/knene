...
\*\*⚠️ 重要说明：index文件不需要文件头注释\*\*

**index文件定义**：
- 文件名为 `index.ts`、`index.tsx`、`index.js`、`index.jsx` 的文件
- 主要用作模块导入导出的聚合文件
- 提供统一对外接口的入口文件

**index文件规范**：
- **禁止添加文件头注释**：index文件不需要 `@fileoverview` 等文件头注释
- **直接开始内容**：文件开头直接是 `import` 或 `export` 语句
- **保持简洁**：专注于导入导出功能，不添加其他注释
...
**例外情况**：
- 如果index文件包含实际的业务逻辑代码（仅仅是导入导出），则需要添加文件头注释
- 需要根据文件的实际功能来判断是否需要注释
...
&nbsp;\*              支持多种布局变体和显示选项，提供完整的配置管理功能。
&nbsp;\*              良好的垂直对齐格式示例，展示多行描述的正确排版方式。
...
1\. \*\*最小化注释原则\*\*：只保留最重要的、最关键的参数注释，避免注释泛滥

2\. \*\*重要性导向\*\*：优先注释对理解代码功能、使用方式有重要影响的参数

3\. \*\*质量胜于数量\*\*：宁缺毋滥，每个注释都必须提供有价值的信息



\*\*必须注释的关键参数类型：\*\*

\- \*\*业务关键参数\*\*：涉及核心业务规则、权限控制、安全约束的参数

\- \*\*复杂配置对象\*\*：结构复杂的自定义类型，包含多个嵌套属性

\- \*\*反直觉参数\*\*：参数名称与实际用途不符或容易产生误解

\- \*\*重要回调函数\*\*：影响组件核心行为的事件处理函数

\- \*\*关键开关参数\*\*：决定功能启用/禁用的重要布尔值



\*\*禁止注释的参数类型：\*\*

\- \*\*基础类型\*\*：`string`、`number`、`boolean` 等基础类型参数

\- \*\*自解释参数\*\*：`title`、`name`、`id`、`url`、`email` 等名称清晰的参数

\- \*\*标准属性\*\*：`className`、`style`、`children` 等通用属性

\- \*\*简单枚举\*\*：取值范围明确且易于理解的枚举参数



```typescript

// ✅ 正确的参数注释格式 - 只注释最关键、最重要的参数
...
&nbsp; downloadUrl?: string // 仅VIP用户可见的下载链接
...
\- \*\*必须注释\*\*：涉及业务逻辑、权限控制、安全约束、复杂配置的核心参数

\- \*\*严禁注释\*\*：基础类型、自解释属性、标准通用属性等显而易见的参数
...
\- 允许最多连续 3 行 `//` 描述；超过 3 行请使用精简说明

\- 优先合并为简洁的一行描述；必要时最多 3 行
