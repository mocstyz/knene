...
export class SessionStorageManager {
   ...
   * 批量设置存储项
   */
  static setMultipleItems(items: Record<string, any>): void {
   ...
   * 批量获取存储项
   */
  static getMultipleItems<T extends Record<string, any>>(keys: (keyof T)[]): Partial<T> {
   ...
   * 设置页面状态
   */
  static setPageState(pageId: string, state: any): void {
    const pageStates = this.getItem<Record<string, any>>('page_states', {})
    if (pageStates) {
      pageStates[pageId] = {
        ...state,
        timestamp: Date.now()
  ...
  static getPageState<T>(pageId: string): T | null {
    const pageStates = this.getItem<Record<string, any>>('page_states', {})
    return pageStates?.[pageId]?.data || null
  }
   ...
   * 设置滚动位置
   */
  static setScrollPosition(pageId: string, position: { x: number; y: number }): void {
    const scrollPositions = this.getItem<Record<string, { x: number; y: number }>>('scroll_positions', {})
    if (scrollPositions) {
   ...
   * 设置表单数据
   */
  static setFormData(formId: string, data: any): void {
    const formDataMap = this.getItem<Record<string, { data: any; timestamp: number }>>('form_data_map', {})
    if (formDataMap) {
      formDataMap[formId] = {
        data,
        timestamp: Date.now()
   ...
   * 设置播放器状态
   */
  static setPlayerState(movieId: string, state: {
    currentTime: number
    duration: number
    volume: number
    playbackRate: number
    ...
    const playerStates = this.getItem('player_states') as Record<string, any> | null || {}
   ...
   * 设置模态框状态
   */
  static setModalState(modalId: string, isOpen: boolean, data?: any): void {
    const modalStates = this.getItem('modal_states') as Record<string, any> | null || {}
    modalStates[modalId] = {
      isOpen,
      data,
      timestamp: Date.now()
  ...
  static cleanupExpiredFormData(): void {
    const formDataMap = this.getItem('form_data_map') as Record<string, { data: any; timestamp: number }> | null
