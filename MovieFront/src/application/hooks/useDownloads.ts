...
import { useDownloadStore, type DownloadTask } from '@/application/stores'
...
const downloadApi = {
  ...
  getActiveDownloads: async (): Promise<DownloadTask[]> => {
    ...
    return downloads.filter(d => d.status === 'downloading' || d.status === 'pending')
  },
  ...
  getDownloadHistory: async (): Promise<DownloadTask[]> => {
    await new Promise(resolve => setTimeout(resolve, 200))
    const downloads = useDownloadStore.getState().downloads
    return downloads.filter(d => d.status === 'completed' || d.status === 'failed')
  },

  // 开始下载
  startDownload: async (movieId: string, quality: string = 'HD'): Promise<DownloadTask> => {
  ...
  // 批量操作
  batchOperation: async (downloadIds: string[], operation: 'pause' | 'resume' | 'cancel' | 'remove'): Promise<void> => {
...
export const useBatchDownloadOperation = () => {
  ...
  return useMutation({
    mutationFn: ({ downloadIds, operation }: { 
      downloadIds: string[]; 
...
export const useCanDownload = (movieId: string) => {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (existingDownload && 
          (existingDownload.status === 'completed' || existingDownload.status === 'downloading')) {
        return false
      }
