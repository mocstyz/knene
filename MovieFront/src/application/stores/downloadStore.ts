...
// 模拟影片数据（用于下载）
const mockMovieData: Record<
  string,
  {
    title: string
    poster: string
    sizes: Record<string, { size: string; url: string }>
  }
> = {
  '1': {
    title: '阿凡达：水之道',
  ...
  '2': {
    title: '流浪地球2',
    poster:
      'https://via.placeholder.com/300x450/cc6600/ffffff?text=Wandering+Earth+2',
    sizes: {
      HD: {
        size: '6.8GB',
        url: 'https://example.com/wandering-earth-2-hd.mkv',
      },
      BluRay: {
        size: '12.8GB',
        url: 'https://example.com/wandering-earth-2-bluray.mkv',
      },
    },
  },
  '3': {
    title: '蜘蛛侠：纵横宇宙',
    poster:
      'https://via.placeholder.com/300x450/cc0066/ffffff?text=Spider-Verse',
    sizes: {
      ...
      (set, get) => ({
        ...
        startDownload: async (
          ...
          const totalSizeBytes =
            parseFloat(sizeData.size) *
            (sizeData.size.includes('GB') ? 1024 * 1024 * 1024 : 1024 * 1024)
        ...
        retryDownload: async (downloadId: string) => {
          ...
          set(state => ({
            ...
            activeDownloads: [
              ...state.activeDownloads.filter(d => d.id !== downloadId),
              {
                ...download,
                status: DownloadStatus.DOWNLOADING,
                progress: 0,
            ...
            failedDownloads: state.failedDownloads.filter(
              d => d.id !== downloadId
            ),
          }))
        ...
        removeDownload: (downloadId: string) => {
          set(state => ({
            ...
            completedDownloads: state.completedDownloads.filter(
              download => download.id !== downloadId
            ),
            failedDownloads: state.failedDownloads.filter(
              download => download.id !== downloadId
          ...
          set(state => {
            const updatedDownloads = state.downloads.map(download => {
              if (download.id === downloadId) {
                ...
                const estimatedTime =
                  speed > 0 ? (download.totalSize - downloadedSize) / speed : 0
            ...
            const updatedActiveDownloads = state.activeDownloads.map(
              download => {
        ...
        simulateDownloadProgress: (downloadId: string) => {
          ...
          const interval = setInterval(() => {
            ...
            const newProgress = Math.min(
              100,
              currentDownload.progress + Math.random() * 5
            )
...
export const selectFailedDownloads = (state: DownloadState) =>
