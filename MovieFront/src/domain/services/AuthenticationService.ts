...
export class AuthenticationService {
   ...
   * 验证用户凭据
   */
  static validateCredentials(email: string, password: string): { isValid: boolean; errors: string[] } {
    const errors: string[] = []
   ...
   * 验证会话令牌
   */
  static validateSessionToken(token: string): { isValid: boolean; userId?: string } {
    try {
   ...
   * 验证密码重置令牌
   */
  static validatePasswordResetToken(token: string): { isValid: boolean; email?: string } {
    try {
      const parts = token.split('_')
