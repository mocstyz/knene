...
export class MovieCatalogService {
  ...
  static getRecommendations(
    ...
    const candidateMovies = allMovies.filter(movie => 
      ...
      !recentlyViewed.includes(movie.id)
    )

    // 计算推荐分数
    const scoredMovies = candidateMovies.map(movie => ({
      movie,
      score: this.calculateRecommendationScore(movie, favoriteGenres, recentlyViewed)
    }))
   ...
   * 获取用户偏好的类型
   */
  private static getUserPreferredGenres(movies: Movie[], favoriteIds: string[]): Map<string, number> {
   ...
   * 获取相似影片
   */
  static getSimilarMovies(targetMovie: Movie, allMovies: Movie[], limit: number = 6): Movie[] {
  ...
  private static calculateSimilarity(movie1: Movie, movie2: Movie): number {
    ...
    // 演员相同
    const commonActors = (movie1.actors || []).filter(actor => (movie2.actors || []).includes(actor))
   ...
   * 验证影片数据
   */
  static validateMovieData(movie: Partial<Movie>): { isValid: boolean; errors: string[] } {
    const errors: string[] = []
    ...
    if (movie.rating !== undefined && (movie.rating < 0 || movie.rating > 10)) {
